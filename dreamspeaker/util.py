...
def dream_to_json(dream_title, dream_claude, dream_original, emoji, database_id):
    """
        ...
        dream_original (str): The original dream transcription.
    ...
    return {
        ...
        "children": [
            ...
            *_text_to_paragraphs(dream_claude),
            _get_children('Original Transcription : ', 'heading_2'),
...
def load_keys(key_file):
        """
            ...
            Returns:
            4-tuple : (NOTION_TOKEN, DATABASE_ID, CLAUDE, API_KEY)
        """
        NOTION_TOKEN = None
        DATABASE_ID = None
        CLAUDE = None
        ...
        with open(key_file, 'r') as f:
            ...
            for key in keys:
                ...
                if key.startswith('NOTION'):
                    NOTION_TOKEN = key.split('=', 1)[1]
                elif key.startswith('DATABASE'):
                    DATABASE_ID = key.split('=', 1)[1]
                elif key.startswith('CLAUDE'):
                    CLAUDE = key.split('=', 1)[1]
                elif key.startswith('API_KEY'):
                    API_KEY = key.split('=', 1)[1]

        return NOTION_TOKEN, DATABASE_ID, CLAUDE, API_KEY
